...
export function registerTools(server: McpServer): void {
  ...
  server.tool(
    'execute_command',
    '컨테이너에서 쉘 명령어를 실행합니다. 터미널/CLI/CMD 명령어, 코드 실행, 파일 작업 등을 수행할 수 있습니다. 반드시 create_container로 먼저 컨테이너를 생성한 후 사용하세요. 명령어 실행이 3초 이상 소요되면 비동기 모드로 전환되어 백그라운드에서 실행됩니다. 비동기 모드로 전환된 경우, 잠시 기다린 후 check_container 또는 execute_command로 결과를 다시 확인해주세요.',
    {
